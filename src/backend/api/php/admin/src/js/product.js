...
function searchProduct(event) {
    ...
    fetch(`?q=product-search?q=${searchTerm}`)
...
function updateProduct(id){
    ...
    fetch(`?q=product-update?id=${id}`)  //Fetch the record from the server
    .then(response => response.json())
    .then(jsonData => {
...
function updateSaveProduct(event){
    ...
    fetch('?q=product-update-save', {      //create a server route/context for this
...
function toggleStatus(productId) {
    ...
    fetch(`?q=product-status?id=${productId}`)
        .then(response => response.json())
        .then(product => {
            ...
            fetch(`?q=product-toggle-status?id=${productId}&status=${newStatusId}`, {
                method: 'PUT',
            })
            .then(response => response.json())
            .then(updatedProduct => {
